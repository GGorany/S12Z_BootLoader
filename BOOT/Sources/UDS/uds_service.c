...
*******************************************************************************/
static void uds_service_10(uint8_t msg_buf[], uint16_t msg_dlc);
...
static void uds_service_85(uint8_t msg_buf[], uint16_t msg_dlc);
static void uds_service_22(uint8_t msg_buf[], uint16_t msg_dlc); 
static void uds_service_2E(uint8_t msg_buf[], uint16_t msg_dlc);
...
static void uds_service_36(uint8_t msg_buf[], uint16_t msg_dlc);
...
static const uds_service_t uds_service_list[] =
{
    ...
    {SID_31, SID_31_MIN_LEN, uds_service_31, TRUE, TRUE, TRUE, TRUE, UDS_SA_NON},//
	{SID_36, SID_36_MIN_LEN, uds_service_36, TRUE, TRUE, TRUE, TRUE, UDS_SA_NON}//
};

#define UDS_SERVICE_NUM (9)

/*******************************************************************************
    Global Varaibles
*******************************************************************************/
bool dis_normal_xmit;
bool dis_normal_recv;
bool Flag_ECU_RESET = FALSE;
 ...
 *     void
 */
static void uds_timer_start(uint8_t num)
{
...
static int16_t eeprom_write(void)         // we should write the rwdata_list which in eeprom to flash
{
	return 0;
}
...
static void uds_service_11(uint8_t msg_buf[], uint16_t msg_dlc)
{
	...
	else if (subfunction == UDS_RESET_TO_APP)
	{
        if (Check_Exist_Application() == TRUE)
	    {
            rsp_buf[0] = USD_GET_POSITIVE_RSP(SID_11);
		    rsp_buf[1] = UDS_RESET_TO_APP;
		    uds_positive_rsp(rsp_buf,2);

		    SET_GOTO_APPLICATION();
		    while(CANComm_GetStateTX() != 0x07){};
            SW_RESET();
	    }
        else
        {
            uds_negative_rsp(SID_11,NRC_GENERAL_REJECT);
        }
	}
	else if (subfunction == UDS_RESET_TO_BT)
	{
		rsp_buf[0] = USD_GET_POSITIVE_RSP(SID_11);
		rsp_buf[1] = UDS_RESET_TO_BT;
		uds_positive_rsp(rsp_buf,2);
...
{
	uint8_t rsp_buf[8] ={0, };
	uint8_t bsc;	// blockSequenceCounter
	uint8_t flashResult;
	...
	flashResult = ProgramFlash(msg_buf, msg_dlc);

    if (noErr == flashResult)
	{
        uds_positive_rsp(rsp_buf, 2);
	}
	else
...
/**
 * uds_dataff_indication - uds first frame indication callback
 *
