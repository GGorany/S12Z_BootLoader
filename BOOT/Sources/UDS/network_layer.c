...
*******************************************************************************/
static void send_flowcontrol(uint8_t flow_st);
...
extern void ZTai_UDS_Send(uint8_t* data, uint8_t Length);
 ...
 *     void
 */
static void nt_timer_start(uint8_t num)
{
...
static void nt_timer_start_wv(uint8_t num, uint32_t value)
{
	if (num >= TIMER_CNT) return;
...
static void nt_timer_stop(uint8_t num)
{
	if (num >= TIMER_CNT) return;
 ...
 */
static int nt_timer_chk(uint8_t num)
{
	if (num >= TIMER_CNT) return 0;
 ...
 *     void
 */
static void recv_singleframe(uint8_t frame_buf[], uint8_t frame_dlc)
{
    uint16_t i, uds_dlc;
	uint8_t service_id;
...
static void send_flowcontrol(uint8_t flow_st)
{
    uint8_t send_buf[UDS_VALID_FRAME_LEN] = {0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA};
 ...
 *     void
 */
static void send_singleframe(uint8_t msg_buf[], uint16_t msg_dlc)
{
	uint16_t i;
    uint8_t send_buf[UDS_VALID_FRAME_LEN] = {0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA};
 ...
 *     void
 */
static void send_multipleframe(uint8_t msg_buf[], uint16_t msg_dlc)
{
	uint16_t i;
	uint8_t send_len;
 ...
 *     void
 */
extern void network_recv_frame(uint8_t func_addr, uint8_t frame_buf[], uint8_t frame_dlc)
{
    uint8_t err;
